...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   React前端     │────▶│   聊天代理服务   │────▶│    n8n Agent    │────▶│ DeepSeek API    │
│   (ChatBot)     │◀────│   (解决CORS)    │◀────│   (工作流)       │◀────│   (大模型)       │
│   :3000         │     │   :3002         │     │   :5678         │     │                 │
└─────────────────┘     └─────────────────┘     └────────┬────────┘     └─────────────────┘
                                                         │
                                                         ▼
...
在PowerShell中依次运行以下命令（每个命令在新终端中运行）：

```powershell
# 1. 启动MCP HTTP服务（新终端，端口3001）
...
# 3. 启动聊天代理服务（新终端，端口3002）
cd E:\web
npm run proxy

# 4. 启动React前端（新终端，端口3000）
cd E:\web
npm start
```

**注意**：聊天代理服务用于解决浏览器 CORS 跨域问题。
